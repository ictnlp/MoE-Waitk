...
def last_n_checkpoints(paths, n, update_based, upper_bound=None, last=None):
    assert len(paths) == 1
    path = paths[0]
    if update_based:
    ...
    if last_key is not None:
        new_entries = sorted(entries)
        # pdb.set_trace()
        for entry in new_entries:
            if entry[0] <= last_key:
                newnew_entries.append(entry)

        if len(newnew_entries) > 5:
            return [
                os.path.join(path, x[1])
                for x in sorted(newnew_entries, reverse=True)[:n]
            ]
        else:
