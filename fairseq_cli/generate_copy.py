...
def _main(cfg: DictConfig, output_file):
    ...
    for sample in progress:
        ...
        test_waitk_lagging = cfg.generation.test_wait_k

        gen_timer.start()
        hypos, gate, head_dict = task.inference_step(
            generator,
            models,
            sample,
        ...
        if iii == 0:
            ...
            G = torch.cat((G, gate), dim=0)

        iii = iii + 1
        """
        if head_dicts['hidden'] is None:
            head_dicts['hidden']=head_dict['hidden']
        else:
            head_dicts['hidden']=torch.cat((head_dicts['hidden'],head_dict['hidden']),dim=0)
        head_dicts['label']=head_dicts['label']+head_dict['label']
        """
        src_len = sample["net_input"]["src_lengths"].tolist()
        tgt_len = [len(h[0]["tokens"]) for h in hypos]
        rw_, gs_ = generate_rw(src_len, tgt_len, k=test_waitk_lagging)
        rw = rw + rw_
        gs = gs + gs_
    ...
    print("AL score: ", al)
    print("DAL score: ", dal)
    print("gate: ", torch.mean(G, dim=0))

    # print(head_dicts['hidden'].size())
    # print(len(head_dicts['label']))

    # np.savetxt("MoE_head_X_2000.npy",head_dicts['hidden'].cpu().numpy())
    # np.savetxt("MoE_head_label_2000.npy",np.asarray(head_dicts['label']) )
