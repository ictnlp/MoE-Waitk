...
class SequenceGenerator(nn.Module):
    ...
    def _generate(
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            src_len = encoder_outs[0][0].size(0)

            # Feed partial source inputs into decoder
            partial_encoder_outs = [
                EncoderOut(
                    encoder_out=encoder_outs[0][0][
                        : min(test_waitk_lagging + step, src_len), :, :
                    ],  # T x B x C
                    encoder_padding_mask=encoder_outs[0][1][
                        :, : min(test_waitk_lagging + step, src_len)
                    ],  # B x T
                    encoder_embedding=encoder_outs[0][2][
                        :, : min(test_waitk_lagging + step, src_len), :
                    ],  # B x T x C
                    encoder_states=encoder_outs[0][3],  # List[T x B x C]
                    src_tokens=encoder_outs[0][4],  # B x T
                    src_lengths=encoder_outs[0][5],  # B x 1
                )
            ]

            lprobs, avg_attn_scores, gate = self.model.forward_decoder(
                tokens[:, : step + 1],
                partial_encoder_outs,
                incremental_states,
                self.temperature,
...
class EnsembleModel(nn.Module):
    ...
    def forward_decoder(
        ...
        return avg_probs, avg_attn, gate
